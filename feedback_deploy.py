...
def predict(model,image):
    ...
    return outputs, result.permute(1, 2, 0).numpy()
...
def feedback(p_image,type="pic"):
    ...
    if confirm_bnt:
        if type == "pic":
            ...
            name = f"./feedback/pictures/{time.time()}.jpg"
            img.save(name)
            with open(f"./feedback/"
                      f"{time.localtime().tm_mday}.md", 'a+') as f:
                f.write(f"## {feedinput} <br>")
                f.write(f"![]({name[11:]})")
                st.success('反馈成功！感谢您的支持！', icon="✅")
        elif type=="video":
            name = f"./feedback/videos/{time.time()}.mp4"
...
def pic_run():
    ...
    if file_up is not None:
        ...
        st.write("")
        output, p_image = predict(model,file_up)
        feedback(p_image)
...
if app_mode == "图像物体识别":
    ...
    st.code(get_code_text("feedback_deploy.py"))
